...
public class ImageProcessor {
	 ...
	 */
	protected static double constrain(double val, double min, double max) {
		if (val < min) {
			return min;
		}
		else if (val > max) {
	...
	/**Jack Cardwell wrote border() and exchange()
	 * April 3, 2015
	 * 
	 * Creates a black border around the picture that is 5 percent from each edge
	 */
	public void border() {
		//use a nested for loop to cycle through all pixels  
		for (int y = 0; y<(image.getHeight());y++) {   
			for (int x = 0; x<image.getWidth();x++){
				//use a conditional to determine the 5% from each side to turn black
				if ( x <=.05*image.getWidth() || x>=.95*image.getWidth() || y<=.05*image.getHeight() || y>=.95*image.getHeight()){
					//Create the new color if it is determined to be part of the border
	...
	/**
	 *exchange all of the colors by turning red into blue, green into red, and blue into green
	 */
	public void exchange(){
		//create a nested for loop to go through all of the pixels of the picture
		for (int y=0; y<image.getHeight(); y++){
			for (int x=0; x<image.getWidth(); x++){
				//get the current color values of red, green, and blue
				Color color=new Color (image.getRGB(x, y));
				//store each of the current color values as formercolor in order to switch them down the line
				int formerred=color.getRed();
				int formergreen=color.getGreen();
				int formerblue=color.getBlue();
				//set formerred to blue, formergreen to red, and formerblue to green
				Color finalcolor= new Color(formergreen, formerred, formerblue);
	...
	public void flip() {
		...
		for (int y = 0; y < image.getHeight(); y++) {
			for (int x = 0; x < image.getWidth(); x++) {
				int y2 = image.getHeight() - 1 - y; // note that indices go 0..height-1
				result.setRGB(x, y2, image.getRGB(x, y));
			}
		}
	 ...
	 */
	public void lens(int cx, int cy, int lensRadius) {
		...
		for (int y = 0; y < image.getHeight(); y++) {
			for (int x = 0; x < image.getWidth(); x++) {
				...
				double dist = 0.1* Math.sqrt((x - cx) * (x - cx) + (y - cy) * (y - cy)) / lensRadius;
				if (dist <= 1) {
