 ...
 */
public class CamPaintExtraCredit extends Webcam {
	...
	public void processImage() {
		...
		if (targetColor!=null){
			...
			if (largestRegion!=null && enableDraw==true){
				//go through all of the points in the largestRegion and make them a different color
				for (Point currentPoint : largestRegion){
				
					//set the color at this point to the colors defined at the start of this class and updated in handleKeyPress
	...
	public void handleKeyPress(char k) {
		...
		else if (k=='g'){ //make the painting color green, the default color
			regionRed=102;
			regionGreen=255;
			regionBlue=102;
		}
		else if (k=='b'){ //make the painting color blue
			regionRed=0;
			regionGreen=128;
			regionBlue=255;
		}
		else if (k=='y'){ //make the color yellow
			regionRed=255;
			regionGreen=255;
			regionBlue=102;
		}
		else if (k=='e'){ //make the color red
			regionRed=255;
			regionGreen=102;
			regionBlue=102;
		}
		else if (k=='a'){ //make the color orange
			regionRed=255;
			regionGreen=153;
			regionBlue=51;
		}
		else if (k=='l'){ //make the color purple
			regionRed=153;
			regionGreen=51;
			regionBlue=255;
		}
		else if (k==' '){  //toggle whether or not the user can paint 
